Feature: Create Activity
  ...
  Scenario: Successfully create a new activity with ID and valid dates
    ...
    When a new activity with ID "A100", name "DesignPhase", start date "01/05/2025" and end date "15/05/2025" is added to the project "ProjectA"
    Then the activity "DesignPhase" with ID "A100" should now be part of the project "ProjectA"

  Scenario: Fail to create activity with end date before start date
    Given a project with the name "ProjectA" is already created
    And project manager is logged in as employee "E123"
    When a new activity with ID "A101", name "InvalidDates", start date "20/05/2025" and end date "10/05/2025" is added to the project "ProjectA"
  ...
  Scenario: Fail to create two activities with same name in same project
    Given a project with the name "ProjectA" is already created
    And project manager is logged in as employee "E123"
    And a new activity with ID "A102", name "DuplicateName", start date "01/06/2025" and end date "10/06/2025" is added to the project "ProjectA"
    When the project manager tries to add another activity with ID "A103", name "DuplicateName", start date "11/06/2025" and end date "20/06/2025" to project "ProjectA"
  ...
  Scenario: Allow same activity name in different projects
    ...
    When a new activity with ID "A201", name "Testing", start date "01/07/2025" and end date "10/07/2025" is added to the project "ProjectA"
    And another activity with ID "B201", name "Testing", start date "05/07/2025" and end date "15/07/2025" is added to the project "ProjectB"
  ...
  Scenario: Assign employee to an activity
    ...
    And project manager is logged in as employee "E123"
    And a new activity with ID "AX01", name "DesignPhase", start date "01/06/2025" and end date "10/06/2025" is added to the project "ProjectX"
  ...
  Scenario: Fail to assign employee to more than 10 activities
    Given employee "E999" is logged in and already assigned to 10 activities
    When the project manager attempts to assign employee "E999" to a new activity with ID "AX99", name "OverflowActivity", start date "01/07/2025" and end date "05/07/2025"
  ...
  Scenario: Cannot assign the same employee twice
    Given a project with the name "ProjectX" is already created
    And project manager is logged in as employee "E123"
    And a new activity with ID "AX02", name "DesignPhase", start date "01/08/2025" and end date "10/08/2025" is added to the project "ProjectX"
  ...
  Scenario: Prevent duplicate activity names in the same project
    ...
    When a new activity with ID "C101", name "Development", start date "01/09/2025" and end date "15/09/2025" is added to the project "ProjectC"
    And the project manager tries to add another activity with ID "C102", name "Development", start date "16/09/2025" and end date "30/09/2025" to project "ProjectC"
